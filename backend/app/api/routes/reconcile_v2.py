...
def start_run(
    ...
    # Scope anomaly detection to THIS run's transactions so each batch pair
    # gets independent, contextual anomalies — not org-wide stale ones.
    # Derive as_of from the data itself so payout_cadence_gap fires correctly
    # for historical data (not relative to today's wall clock).
    run_txns = source_txns + bank_txns
    run_dates = [t.date for t in run_txns if t.date]
    as_of_date = max(run_dates) if run_dates else date.today()
    current_month_date = as_of_date.replace(day=1)
    detected = detect(run_txns, current_month=current_month_date, as_of=as_of_date)
...
def list_runs(
    ...
    from app.models.transaction import UploadBatch
    # Get the set of valid batch IDs so we can flag orphaned runs
    valid_batch_ids = {
        r[0] for r in db.query(UploadBatch.id).filter(UploadBatch.org_id == org_id).all()
    }
    runs = db.query(ReconciliationRun).filter(ReconciliationRun.org_id == org_id)                                       .order_by(ReconciliationRun.created_at.desc())                                       .limit(50).all()
    result = []
    for r in runs:
        # Skip runs whose batches have been deleted (orphaned runs)
        if r.source_batch_id not in valid_batch_ids or r.bank_batch_id not in valid_batch_ids:
            continue
        result.append(RunSummaryOut(
...
def get_run(
    ...
    # Anomalies are scoped to transactions in this run's source + bank batches
    run_txn_ids = [
        row[0] for row in db.query(Transaction.id).filter(
            Transaction.batch_id.in_([run.source_batch_id, run.bank_batch_id]),
            Transaction.org_id == org_id,
        ).all()
    ]
    # An anomaly belongs to this run if ANY of its transaction_ids appear in the run
    # We load all non-dismissed org anomalies and filter in Python (simpler than JSON query)
    all_anomalies = db.query(Anomaly).filter(
        Anomaly.org_id == org_id, Anomaly.status != "dismissed"
    ).order_by(Anomaly.detected_at.desc()).limit(200).all()

    run_txn_set = set(run_txn_ids)
    anomalies = [
        a for a in all_anomalies
        if any(tid in run_txn_set for tid in (json.loads(a.transaction_ids) or []))
    ]
...
def scan_anomalies(
    ...
    all_dates = [t.date for t in all_org_txns if t.date]
    as_of_scan = max(all_dates) if all_dates else date.today()
    detected = detect(all_org_txns, current_month=as_of_scan.replace(day=1), as_of=as_of_scan)
    ...
    for a in detected:
        ...
        new_count += 1
    db.commit()
    return {"new_anomalies": new_count, "scanned": len(detected)}def _run_detail_dict(run: ReconciliationRun, matches: list, anomalies: list) -> dict:
    return {
        "id": run.id,
        "org_id": run.org_id,
        "source_batch_id": run.source_batch_id,
